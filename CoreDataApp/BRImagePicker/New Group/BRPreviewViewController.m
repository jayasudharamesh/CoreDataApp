...
- (void)viewDidLoad {
    ...
    newcontext=appDelegate.persistentContainer.viewContext;
...
- (IBAction)donePressed:(UIButton *)sender {
    ...
    BR_ImageInfo *imageInfo;
    BR_AssetsInfo *assetsInfo;
    NSMutableArray *imageInfoArray = [NSMutableArray array];
    for (assetsInfo in self.selectedAssetsArray) {
        imageInfo = [[BR_ImageInfo alloc] initWithImage:assetsInfo.originalImage andCaption:assetsInfo.caption];
    ...
    NSData *imageData;
    imageData=[[NSData alloc] initWithData:UIImageJPEGRepresentation((imageInfo.image), 1.0)];
    
    
    newtransaction = [NSEntityDescription insertNewObjectForEntityForName:@"NewEntity" inManagedObjectContext:newcontext];
    
    
    
    [newtransaction setValue:[NSString stringWithFormat:@"%@", imageInfo.caption] forKey:@"captionwithImage"];
    [newtransaction setValue:imageData forKey:@"addImage"];
    ...
    [newcontext refreshObject:newtransaction mergeChanges:YES];
